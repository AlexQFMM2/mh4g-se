...
$DexExe = Join-Path $DexDir "MH4G Dex.exe"
$Csc = Join-Path $env:WINDIR "Microsoft.NET\Framework\v4.0.30319\csc.exe"

if (-not (Test-Path -LiteralPath $DexExe -PathType Leaf)) {
    throw "MH4G Dex.exe was not found: $DexExe"
}
if (-not (Test-Path -LiteralPath $Csc -PathType Leaf)) {
    throw "32-bit .NET Framework compiler was not found: $Csc"
}

$BuildDir = Join-Path ([System.IO.Path]::GetTempPath()) ("mh4g-dex-dump-" + [Guid]::NewGuid().ToString("N"))
$RunnerExe = Join-Path $BuildDir "MH4GDexDump.exe"
